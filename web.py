...
# ========== 页面配置 ==========
st.set_page_config(page_title='韧性强度预测', page_icon='🧪', layout='wide')
...
with st.sidebar.container():
    st.header('说明')
    st.write('此页面为韧性强度预测系统。')
    st.write('当前未加载真实模型，所有预测结果固定为 **4.75**。')
    st.write('输入框、按钮、图表功能保持原样，用于展示界面布局。')
...
if predict_btn:
    ...
    <div class='card' style='text-align:center'>
      ...
      <div style='color:#475569;margin-top:6px;font-size:13px'>固定输出</div>
    </div>
    """
    ...
    ax.set_title('SHAP 特征贡献')
    ax.set_xlabel('贡献度')
    st.pyplot(fig)

else:
    with col2:
        st.info('输入参数后点击“预测”可获得结果 (固定为4.75)。')

st.markdown('<hr />', unsafe_allow_html=True)
st.markdown('**说明：** 本系统展示模型预测界面布局及交互效果，不执行真实机器学习推理。', unsafe_allow_html=True)
